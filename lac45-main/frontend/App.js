...
export default function App() {
  ...
  const loadVideos = async () => {
    try {
      const response = await fetch('https://pubcontrol-hub.preview.emergentagent.com/api/videos');
  ...
  const loadNews = async () => {
    try {
      const response = await fetch('https://pubcontrol-hub.preview.emergentagent.com/api/news');
      const data = await response.json();
      setNews(data.news || []);
    } catch (error) {
  ...
  const handleLogin = async (email, password) => {
    setLoading(true);
    try {
      const response = await fetch('https://pubcontrol-hub.preview.emergentagent.com/api/auth/login', {
        method: 'POST',
        headers: {
  ...
  const handleRegister = async (userData) => {
    setLoading(true);
    try {
      const response = await fetch('https://pubcontrol-hub.preview.emergentagent.com/api/auth/register', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
